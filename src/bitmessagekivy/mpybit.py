...
class Navigatorss(MDNavigationDrawer):
    """Navigators class contains image source, title, and logo."""
...
class Inbox(Screen):
            ...
            for mail in queryreturn:
                ...
                data.append({
                    ...
                    'secondary_text': mail[5][:10] + '...........' if len(
                        ...
                        third_text) > 25 else third_text, 'receivedTime': mail[6]})
            for item in data:
    ...
    def delete(self, data_index, instance, *args):
        ...
        sqlExecute("UPDATE inbox SET folder = 'trash' \
            WHERE received = {};".format(data_index))
...
class MyAddress(Screen):
    ...
    def __init__(self, *args, **kwargs):
        """Method calling super and schedule Clock."""
...
class AddressBook(Screen):
    ...
    def __init__(self, *args, **kwargs):
        """Addressbook init calling super and schedule clock."""
...
class RV(RecycleView):
    ...
    def __init__(self, **kwargs):
        """Recycling init method."""
        super(RV, self).__init__(**kwargs)


class DropDownWidget(BoxLayout):
    """Adds Dropdown Widget."""
        ...
        if sendMessageToPeople:
            if toAddress != '' and subject and message:
                ...
                if status == 'success':
                    ...
                    state.msg_counter_objs = self.parent.parent.parent.parent.parent.parent.children[
                        0].children[2].children[0].ids
...
class Sent(Screen):
            ...
            for mail in queryreturn:
                ...
                self.data.append({
                    ...
                    'secondary_text': mail[2][:10] + '...........' if len(
                        ...
                        third_text[:25] + '...!') if len(
                        third_text) > 25 else third_text, 'lastactiontime': mail[6]})
    ...
    def delete(self, data_index, instance, *args):
        ...
        sqlExecute(
            "UPDATE sent SET folder = 'trash' WHERE lastactiontime = {};".format(data_index))
        self.ids.ml.remove_widget(instance.parent.parent)
        self.update_trash()
...
class Trash(Screen):
    ...
    def __init__(self, *args, **kwargs):
        """Method calls the super function and clockscheduler."""
    ...
    def init_ui(self, dt=0):
        ...
        inbox = sqlQuery("SELECT toaddress, fromaddress, subject, message \
            from inbox WHERE folder = 'trash' \
            and fromaddress = '{}';".format(state.association))
        sent = sqlQuery("SELECT toaddress, fromaddress, subject, message \
            from sent WHERE folder = 'trash' \
            and fromaddress = '{}';".format(state.association))
...
class Create(Screen):
    ...
    def __init__(self, **kwargs):
        """Method creates the creating widgets."""
        super(Create, self).__init__(**kwargs)
        widget_1 = DropDownWidget()
        widget_1.ids.txt_input.word_list = [addr[1] for addr in sqlQuery(
...
class NavigateApp(App):
    ...
    def getCurrentAccountData(self, text):
        ...
        state.trash_count = str(
            sqlQuery("SELECT (SELECT count(*) FROM  sent \
                where fromaddress = '{0}' and  folder = 'trash' )+\
                (SELECT count(*) FROM inbox where fromaddress = '{0}' \
                and  folder = 'trash') \
                AS SumCount".format(state.association))[0][0])

        state.draft_count = str(
            sqlQuery("SELECT COUNT(*) FROM sent \
                WHERE fromaddress = '{}' and \
    ...
    def restart(self, *args):
        """Method is used to set transition direction."""
    ...
    def mail_count(self, text):
        """Counting the numbers of mails ."""
        if state.association == '':
            if BMConfigParser().addresses():
                state.association = BMConfigParser().addresses()[0]
        if text == 'Sent':
            state.sent_count = str(
                sqlQuery("SELECT COUNT(*) FROM {0} \
                    WHERE fromaddress = '{1}' and folder = '{0}' ;\
                    ".format(text.lower(), state.association))[0][0])
            ...
            state.trash_count = str(
                sqlQuery("SELECT(SELECT count(*) FROM  sent where \
                    fromaddress = '{0}' and  folder = 'trash' )+ \
                    (SELECT count(*) FROM inbox where fromaddress = '{0}' \
                    and  folder = 'trash') AS \
                    SumCount".format(state.association))[0][0])
            return state.trash_count
        elif text == 'Draft':
            state.draft_count = str(
                sqlQuery(
                    "SELECT COUNT(*) FROM sent WHERE \
                    fromaddress = '{1}' and folder = '{0}' \
                    ;".format(text.lower(), state.association))[0][0])
...
class GrashofPopup(Popup):
    ...
    def show_error_message(self):
        ...
        self.dialog.add_action_button(
            "ok", action=lambda *x: self.dialog.dismiss())
        self.dialog.open()
...
class IconLeftSampleWidget(ILeftBodyTouch, MDIconButton):
    """Icon Left Sample Widget."""

    pass


class IconRightSampleWidget(IRightBodyTouch, MDCheckbox):
    """Icon Right Sample Widget."""

    pass


class NavigationDrawerTwoLineListItem(TwoLineListItem, NavigationDrawerHeaderBase):
    """Navigation Drawer in Listitems."""

    address_property = StringProperty()

    def __init__(self, **kwargs):
        """Init method calls super and schedule clock."""
...
class MailDetail(Screen):
    ...
    def init_ui(self, dt=0):
        ...
        if state.detailPageType == 'sent':
            data = sqlQuery(
                ...
                status, ackdata from sent \
                where lastactiontime = {};".format(state.sentMailTime))
            state.status = self
            state.ackdata = data[0][5]
            self.assign_mail_details(data)
        elif state.detailPageType == 'inbox':
            data = sqlQuery("select toaddress, fromaddress, subject, message\
                from inbox where received = {};".format(state.sentMailTime))
            self.assign_mail_details(data)
    ...
    def delete_mail(self):
        """Deleting Mail From Inbox, Sent, trash."""
        msg_count_objs = self.parent.parent.parent.parent.parent.children[2].children[0].ids
        if state.detailPageType == 'sent':
            sqlExecute("UPDATE sent SET folder = 'trash' \
                WHERE lastactiontime = {};".format(state.sentMailTime))
    ...
    def inbox_reply(self):
        """Method is used for replying inbox messages."""
        data = sqlQuery(
    ...
    def copy_sent_mail(self):
        """Method is used for copying sent mail to the composer."""
        pass
...
class Draft(Screen):
    ...
    def delete_draft(self, data_index, instance, *args):
        """Method is used to delete draft message permanently."""
        sqlExecute(
            "DELETE FROM  sent WHERE lastactiontime = '{}';".format(
                data_index))
        try:
            msg_count_objs = self.parent.parent.parent.parent.children[2].children[0].ids
        except Exception as e:
    ...
    def draft_msg(self, src_object):
        """Method is used for saving draft mails."""
